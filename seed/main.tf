...
resource "google_compute_instance" "seed001" {
 name         = "dmlc-seed-${random_id.instance_id.hex}"
...
metadata_startup_script = file("${var.scbootstrap}/initdm.sh")
 metadata = {
...
resource "google_compute_firewall" "seed" {
 ...
 allow {
   protocol = "tcp"
   ports    = ["5000","8500","8301","8300","8302","8140","4141","3000","9090","9091","9093"]
 }
}
